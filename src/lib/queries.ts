...
export const getAvailableItems = (timeMin: Date, timeMax: Date) => axios.get(`/api/items?range=[0,999]&timeMin=${timeMin.toISOString()}&timeMax=${timeMax.toISOString()}`)
...
export const getMonthReservations = (date: Date) => axios.get(`/api/reservations?range=[0,999]&month=${date.toISOString()}`)
