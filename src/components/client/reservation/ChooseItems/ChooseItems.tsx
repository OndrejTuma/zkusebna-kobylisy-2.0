...
type StructuredCategory = ReservationItemCategory & {
...
const renderStructuredCategory = ({ id, title, items, subcategories }: StructuredCategory) => (
  <StyledTreeItem key={id} nodeId={id} label={title}>
    ...
    {items.map(({ id, title, code, price }) => (
      <StyledTreeItem key={id} nodeId={id} label={(
        <strong>{title} {code ? `(${code})` : ''} {price}</strong>
      )}/>
    ))}
  </StyledTreeItem>
)

const ChooseItems = ({categories, items}: Props) => {
  const structuredCategories: StructuredCategory[] = categories.filter(({ parent_id }) => !parent_id).map(category => getSubcategoriesAndItems(category, categories, items))
